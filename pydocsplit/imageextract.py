...
class ImageExtractor:
    ...
    @staticmethod
    def _expand_page_sequence(value, delim='-'):
        if isinstance(value, (int, long)):
            return [value, ]
        if delim in value:
            pair = [int(x.strip()) for x in value.split(delim, 1) if x.isdigit()]
            if len(pair) != 2:
                return []
    ...
    def pages_arg(self, pages):
        result = []
        [result.extend(self._expand_page_sequence(x)) for x in pages]
    ...
    def convert(self, pdf, size, format, previous=None):
        ...
        if self.options.get('pages'):
            pages = self.pages_arg(self.options['pages'])
        else:
